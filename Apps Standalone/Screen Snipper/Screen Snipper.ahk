...
; ============================================================================
;
; I did copy this tool from https://www.autohotkey.com/boards/viewtopic.php?f=83&t=115622
; It's not perfect at all, but it works quite well for my needs.
;
; [USAGE]
;	 Win & drag to snip and copy the selected area.
;	 Ctrl + Win & drag to copy only.
;	 Alt + Win & drag to save only.
;	 Shift + Win & drag to OCR only.
...
/*********** User Instruction ***********
	...
	Win   + LButton Drag:		    Snip and copy
	Ctrl  + Win + LButton Drag:	  	Copy only
	Alt   + Win + LButton Drag:		Save only
	Shift + Win + LButton Drag:		OCR only

	RButton:		    		Open Options
	Esc:						Close current
	Ctrl + Alt					Show/Hide all
	Alt + Ctrl					Close all
...
_SetupTrayMenu() {   
    ...
    A_TrayMenu.Disable("OCR Only:         Shift + Win + LButton")
}
...
;{-----------------------------------------------
;
STOAGE_PATH := "Snipper Images"
Settings_SavePath_Image := "C:\Users\" A_UserName "\pictures\" STOAGE_PATH "\" ; GetFullPathName('.\Snipper - Images\')
...
ContextSnipMenuData.Push('INFO', '', '', 'OCR:   Extract Text', '', 'COPY:  Clipboard', '', 'SAVE:  ' Settings_SavePath_Image_Ext ' File')
For Index, Item in Extensions
	For Appl, AppObj in Item.OwnProps()
	{
		If Appl != PrevApp
...
ContextSnipMenuData.Push('', '', 'Destroy')
ContextSnipMenuData.Push('', '', 'Destroy All')
...
; 	Area := SelectScreenRegion('LButton')
; 	If (Area.W > 8 and Area.H > 8)
; 		SnipArea(Area, false, false, true, SnipVisible, &guiSnips)
; 	sleep 1000 ; i mannualy click on the snippet to activate it
; 	Snip2File(false, Settings_SavePath_Image, , Settings_SavePath_Image_Ext, , Settings_Image_Quality)
; }


~^Alt:: ;	<-- Toggle Show/Hide of Snips
{
	Global SnipVisible
	...
	If !SnipVisible {
		if guiSnips.Count < 1 
			SnipVisible := true ; make sure to never snip while invisable
		For Hwnd, guiSnip in guiSnips
			guiSnip.GuiObj.Hide
	}
}
...
DestroyAllSnips() {
	For Hwnd, guiSnip in guiSnips
		guiSnip.GuiObj.Destroy()
	Reload() ; To prevent errors
}
...
~RButton::	;	<-- @@ Click for Context Menu
{
	...
	If WinGetTitle('ahk_id' OutputVarWin) = 'SnipperWindow'
	{
		...
		ContextSnipMenu.Rename('1&', ContextSnipMenuData[1] ':    ' W - 6 ' Width ' H - 6 ' Height')
...
ContextSnipMenu_Handler(ItemName, ItemPos, MyMenu)
{
	...
	If ContextSnipMenuData[ItemPos].HasProp('Func')
		ContextSnipMenuData[ItemPos].Func.Call()
	; Standard Actions
	Switch ItemPos
	{
		; Case 1: WinGetPos(&X, &Y, &W, &H, 'A'), SettingsDialog({ X: X, Y: Y, W: W, H: H })
		Case 4: Snip2Clipboard() CloseSnip() OCR.GetOcrFromClipboard()
		Case 6: Snip2Clipboard() CloseSnip()
		Case 8: Snip2File(, Settings_SavePath_Image, true, Settings_SavePath_Image_Ext, , Settings_Image_Quality) CloseSnip()
		Case 11: CloseSnip()
		Case ContextSnipMenuData.Length: DestroyAllSnips()
	}
}
;}
...
;
;{ SelectScreenRegion
SelectScreenRegion(Key, Color := '0011ff', Color_Inactive := 'Red', Transparent := 80)
{
	; Initialize
	...
	{
		; If GetKeyState('Shift', 'P') or false and (W > 60 and H > 35)
		; {
		; 	guiSSR.GetPos(&X, &Y, &W, &H)
...
WM_ACTIVATE(wParam, lParam, msg, hwnd)
{
	...
	If WinGetTitle('ahk_id' hwnd) = 'SnipperWindow'
		If (Activated := wParam << 48 >> 48)
			SnipWinBorderColor(guiSnips[hwnd].GuiObj, 'Green') ; Highlighted color
		Else
			SnipWinBorderColor(guiSnips[hwnd].GuiObj, 'Blue') ; Default border color
}
;}
;{ SnipArea
SnipArea(Area, SetClipboard := false, save := false, CreateWindow := true, ShowWindow := true, &ObjMap?, BorderColorOut := 'Blue', BorderColorIn := 'White')
{
	GDIp.Startup()
	pBitmap := GDIp.BitmapFromScreen(Area)
	If save {
        Info("Saving snippet...", 3000)
		If !FileExist(Settings_SavePath_Image)
			DirCreate(Settings_SavePath_Image)

		fileName := FormatTime(, 'dd-MM-yyyy @ HH-mm-ss') "." Settings_SavePath_Image_Ext
		fullPath := Settings_SavePath_Image fileName
		GDIp.SaveBitmapToFile(pBitmap, fullPath)
		Run("explorer.exe /select,`"" fullPath "`"")
		
		WinWait(STOAGE_PATH)
		Sleep(100)
		WinActivate(STOAGE_PATH)
		Sleep(1500)

	}
	; Snip2File(false, Settings_SavePath_Image, true, Settings_SavePath_Image_Ext, , Settings_Image_Quality)
	If SetClipboard
...
Snip2File(Borders := false, SavePath?, FileNameOnly?, FileExt?, Hwnd?, Quality?)
{
	...
	Run("explorer.exe /select,`"" SavePath FileName "`"")
	CloseSnip()
