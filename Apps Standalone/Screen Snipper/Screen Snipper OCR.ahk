...
; Adds a snip-to-clipboard OCR flow on top of the vendored OCR library included above,
; instead of keeping a second, drifting copy of the whole library in this file.
OCR.DefineProp("TakePrintscreenAndOcsToClipboard", {Call: OCR_TakePrintscreenAndOcsToClipboard})
OCR.DefineProp("GetOcrFromClipboard", {Call: OCR_GetOcrFromClipboard})

OCR_TakePrintscreenAndOcsToClipboard(self) {
	...
	ClipWait(1, 1)
	Sleep 100
	self.GetOcrFromClipboard()
}

OCR_GetOcrFromClipboard(self) {
	 ...
	 try {
		result := self.FromBitmap(hBitmap, 2)
		text := rearrangeOCRresult(result)

		A_Clipboard := text
        Info("Copied`n" text, 5000)
	 }
	 catch
		Info "Error: Clipboard does not hold an image"
