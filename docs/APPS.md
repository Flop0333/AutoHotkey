...
| [Control Dashboard](../Dashboards/Control%20Dashboard/Dashboard.ahk) | Auto through logging initialization | Open from the startup tray, Macro Board, or Age of Efficiency command `AL` to inspect and copy log entries. | `Dashboard.ahk` renders `Logs/errors.log`; the active log is archived on suite startup. Overview, Processes, Logs, Profiles, and Health carry suite state, process control, and profile switching; the Tests section is still being added. See the [design record](decisions/control-dashboard.md). |
