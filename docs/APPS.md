...
| [Notion Board](../Dashboards/Notion%20Board/Notion%20Board.ahk) | Auto, desktop | `CapsLock+N` brings it forward or sends it behind other windows; close and minimize do the latter too. | Shows the board from the `NotionBoardUrl` secret in a focused WebView2 view that removes Notion's header, sidebar, and page-comments panel. Its fixed, git-ignored profile folder (`Dashboards/Notion Board/Profile/`) preserves the login across suite restarts. A missing secret shows a notice instead of a blank window. |
