...
| [Macro Board](../Dashboards/Macro%20Board/Macro%20Board.ahk) | Auto | `CapsLock+Space` opens a Stream Deck-style action grid. | Configure common and profile buttons in the entry point; windowed geometry is stored in the ignored `Dashboards/Macro Board/window settings.ini`. |
| [Logger](../Dashboards/Logger/Logger%20Host.ahk) | Auto through logging initialization | Small notification host that surfaces new structured errors in the Control Deck's retro style. Left-click opens Logs; right-click or the close glyph dismisses. | `Logging.ahk` is the public facade; `Controller.ahk` owns popup state and `Popup Renderer.ahk` paints it with the Control Deck artwork. |
| [Control Deck](../Dashboards/Control%20Deck/Dashboard.ahk) | Auto, hidden | `CapsLock+LWin` shows it, or hides it when it is the active window. Also opens from the startup tray, Macro Board, or Age of Efficiency command `D`. Logger notifications open Logs; `Test` and `Tests/Run-Tests.ahk` open Tests and start a run. | Overview, Processes, Logs, Tests, Profiles, and Health share one host. It starts hidden with the suite so opening it is instant, pauses its polling while hidden, and is still started on demand if it is not running. Keys `1`–`6` switch sections, `R` runs tests, `/` focuses the log filter, and `Esc` closes log details. The title bar shows the checked-out branch: click it to switch branch (`main` is listed first; uncommitted changes are stashed with a message or discarded first), and click the `↓`/`↑` counts to pull, then push. A branch that was never pushed shows its unpushed commits as `↑` and is published with tracking on the first push. |
| [Notion Board](../Dashboards/Notion%20Board/Notion%20Board.ahk) | Auto, desktop | `CapsLock+N` brings it forward or sends it behind other windows; close and minimize do the latter too. Its restored position and size persist across restarts. | Shows the board from the `NotionBoardUrl` secret in a focused WebView2 view that removes Notion's header, sidebar, and page-comments panel. Its fixed, git-ignored browser-data folder (`Dashboards/Notion Board/Browser Data/`) preserves the login, while the ignored `Dashboards/Notion Board/window settings.ini` stores only windowed geometry. A missing secret shows a notice instead of a blank window. |
