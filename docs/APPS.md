...
| [Control Dashboard](../Dashboards/Control%20Dashboard/Dashboard.ahk) | Auto through logging initialization | Open from the startup tray, Macro Board, or Age of Efficiency command `AL` to inspect and copy log entries. | `Dashboard.ahk` renders `Logs/errors.log`; the active log is archived on suite startup. Overview, Processes, Logs, Tests, Profiles, and Health carry suite state, process control, test runs, and profile switching. See the [design record](decisions/control-dashboard.md). |
