...
| [Notion Board](../Dashboards/Notion%20Board/Notion%20Board.ahk) | Auto, hidden | `CapsLock+N` shows or hides it; closing the window hides it instead of exiting. | Shows the board from the `NotionBoardUrl` secret in its own WebView2 window with a fixed, git-ignored profile folder (`Dashboards/Notion Board/Profile/`), so a single manual login survives a suite restart. A missing secret shows a notice instead of a blank window. |
